...
def fetch_pcls_archive() -> Path:
    ...
    archive = Path(
        pooch.retrieve(
            ...
            known_hash=PCLS_ARCHIVE_HASH,  # pins raw .tar.gz bytes; safe because Figshare serves byte-stable statics. For a non-deterministic archive pin the extracted content instead (see data.md).
